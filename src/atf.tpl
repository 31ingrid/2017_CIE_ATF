//Make sure to change this in BSAI or GOA. init_bounded_dev_vector fmort_dev(styr,endyr,-3,3,1)  
//was -3,3,1 change IS 2015
//                      atfbsai2.tpl
...
//#length bins - 25 bins 26 numbers to define        10 16 18 20 22 24 26 28 30 32 34 36 38 40 43 46 49 52 55 58 61 64 67 70 75 110 I moved last one up to super large so bins can be 75 (inclusive)                         
...
//  init_bounded_number_matrix srv_params_m(1,nsurv,1,nsel_params,sel_LB_m,sel_UB_m)   
 ...
 //phase of -1 means M is fixed   
  ...
  init_bounded_dev_vector fmort_dev(styr,endyr,-3,3,1)  //was -3,3,1 change IS 2015   GOA -5,3.5,1
...
// cout<<"obs_p_fish(1,1)"<<obs_p_fish(1,1)<<std::endl; 
...
FUNCTION get_selectivity
 ...
 for(k=1;k<=2;k++)
   {
     avgsel_fish(k)=log(mean(mfexp(log_selcoffs_fish(k))));    //This is just a normalizing conditioning to a constant…the penalty goes to zero and is just a condition…i.e., that the mean log selectivity is zero
   }
 ...
 for(k=1;k<=2;k++)
   {
     avgsel_srv2(k)=(mean((sel_srv(k,2))));    //
   }
// cout<<"log_selcoffs_fish"<<log_selcoffs_fish<<std::endl; 
...
FUNCTION compute_spr_rates
 ...
 // Now do plus group
  ...
  sprpen   += 200.*square((SBF30/SB0)-0.30);   //Also not worth including as its just a condition…could solve outside of minimizer of ADMB in the code (i.e., not have them as ADMB parameters.
    ...
    for(k=1;k<=2;k++)
    {
      for (i=1; i <= nobs_fish; i++)
      {
        ...
        //fishery length likelihood fitting
          length_like(1) -= nsamples_fish(k,i)*(offset_const+obs_p_fish(k,i))*log(pred_p_fish(k,ii)+offset_const); //was offset_const  for GOA and for GOA the offset_const was 0.0001. For BSAI do this and set offset_const to 0.001.
      }
    }
...
//  double var_tmp; for (i=1;i<=nobs_srv(3);i++) { var_tmp = 2.*square(log(obs_srv(3,i))*cv_srv(3,i)); surv_like(3) += square(log(obs_srv(3,i)+.01)-log(pred_srv(3,yrs_srv(3,i))+.01))/var_tmp; }
   ...
   catch_like=norm2(log(catch_bio+offset_const)-log(pred_catch+offset_const));      //was offset_const - set to offset_const for GOA
 ...
 //here are taking the sum of squares of the second differences  
  ...
  // Phases less than 3, penalize High F's
    if (current_phase()<2)
    {
       ...
       fpen=10.*norm2(mfexp(fmort_dev+log_avg_fmort)-offset_const); //was offset_const
    }
    else
    {
      fpen=fpen_mult(1)*norm2(mfexp(fmort_dev+log_avg_fmort)-offset_const);   //was offset_const   //0.001 for BSAI and 0.01 for GOA see fpen_mult values  
    }
  ...
  obj_fun += catch_err_like*like_wght(4)*catch_like;        // large emphasis to fit observed catch 
  obj_fun += fpen;   
  obj_fun += sprpen;   
...
REPORT_SECTION
  ...
  report << "length_like_survey"<<endl<<length_like<<endl;        //fishery is 1 survey is 2 shelf 3 slope 4 ai 
...
//full selection F is not 13 adn 13 - 8 for females, 21 for males.
